...
const LieTest = (props) => {
  ...
  useEffect(() => {
    const fetchDataAndComputeScores = async () => {
      try {
        ...
        console.log("professor data: ", professorData); //printing professor data
        const updatedLeaderboard = Leaderboard.map((professor) => {
          const matchingProfessor = professorData.find(
            (p) => p.name === professor.name
          );
          if (matchingProfessor) {
            const score =
              (matchingProfessor.trueResponses / matchingProfessor.totalTests) *
              100;
            return {
              ...professor,
  ...
  const onClickForward = () => {
    ...
    axios
      .post("http://localhost:3001/update-professor", {
        professorIndex,
        hasSeenProfessor,
      })
      .then((response) => {
        console.log(
          "Professor score updated successfully:",
          response.data,
          professorIndex,
          hasSeenProfessor
        );
    ...
    if (index + 1 === imgList.length) {
      props.setResultsScores(trueCount);
      setEndOfTest(true);
    } else {
      setIndex(index + 1);
    }
